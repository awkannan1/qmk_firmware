...
static PWMConfig pwmCFG_breathing = {0xFFFF,                          /* 10kHz PWM clock frequency  */
                                     256,                             /* PWM period (in ticks) 1S (1/10kHz=0.1mS 0.1ms*10000 ticks=1S) */
...
bool is_breathing(void) { return PWMD3.config == &pwmCFG_breathing; }
...
#define breathing_max()                                 \
    do {                                                \
        breathing_counter = breathing_period * 256 / 2; \
    } while (0)
...
void breathing_interrupt_enable(void) {
    ...
    pwmEnableChannelI(&PWMD3, 0, PWM_FRACTION_TO_WIDTH(&PWMD3, 0xFFFF, 0xFFFF));
    chSysUnlockFromISR();
}
...
static inline uint16_t scale_backlight(uint16_t v) { return v / BACKLIGHT_LEVELS * kb_backlight_config.level; }
...
static void breathing_callback(PWMDriver *pwmp) {
    ...
    chSysLockFromISR();
    pwmEnableChannelI(&PWMD3, 0, PWM_FRACTION_TO_WIDTH(&PWMD3, 0xFFFF, duty));
    chSysUnlockFromISR();
}
